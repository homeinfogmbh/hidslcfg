...
class Configurator():
    ...
    VPN_INST = '/usr/bin/tar xzf {arch} -C /etc/openvpn'
    ...
    def __enter__(self):
        ...
        pacmanconf_path = join(self._tmpdir, self.PACMAN_CONF_SRC)
    ...
    def sudo(self, cmd):
        ...
        result = run('/usr/bin/sudo -n {cmd}'.format(cmd=cmd), shell=True)
    ...
    def configure_pacman(self):
        ...
        if self._tmpdir is not None:
            chdir(self._tmpdir)
            self.sudo(self.PACMAN_INST.format(
                src=self.PACMAN_CONF_SRC, dst=self.PACMAN_CONF_DST))
        else:
    ...
    def configure_openvpn(self):
        ...
        if self._tmpdir is not None:
            ...
            self.sudo('/usr/bin/rm /etc/openvpn/*')
            # Unpack openVPN archive
            chdir(self._tmpdir)
            self.sudo(self.VPN_INST.format(arch=self.VPN_ARCHIVE))
        else:
            raise OutsideOfWithStatement('Not within with statement')
    ...
    def configure_hostname(self):
        ...
        if self._tmpdir is not None:
            chdir(self._tmpdir)
            hostname = '{tid}.{cid}'.format(tid=self._tid, cid=self._cid)
            with open(self.HOSTNAME_FILE_SRC, 'w') as hostname_tmp:
                hostname_tmp.write(hostname)
            self.sudo(self.HOSTNAME_INST.format(
